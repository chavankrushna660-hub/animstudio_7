...
export const interpolateTwoObjects = (startObj: VectorObject, endObj: VectorObject, t: number): VectorObject => {
  ...
  // Interpolate VST (Vector Smart Transform) state across keyframes
  let vstState = startObj.vstState;
  if (startObj.vstState || endObj.vstState) {
    const startV = startObj.vstState;
    const endV = endObj.vstState;
    const identityT: Transform = { x: 0, y: 0, rotation: 0, scaleX: 1, scaleY: 1, skewX: 0, skewY: 0, rotateX: 0, rotateY: 0, perspective: 0 };

    if (startV?.active && endV?.active) {
      const sT = startV.transform || identityT;
      const eT = endV.transform || identityT;
      let interpolatedPivot = startV.pivot;
      if (startV.pivot && endV.pivot) {
        interpolatedPivot = {
          x: Number((startV.pivot.x + t * (endV.pivot.x - startV.pivot.x)).toFixed(2)),
          y: Number((startV.pivot.y + t * (endV.pivot.y - startV.pivot.y)).toFixed(2))
        };
      } else if (endV.pivot && t > 0.5) {
        interpolatedPivot = endV.pivot;
      }
      vstState = {
        ...startV,
        pivot: interpolatedPivot,
        transform: {
          ...interpolateTransform(sT, eT, t),
          mirrorX: t > 0.5 ? endV.transform?.mirrorX : startV.transform?.mirrorX,
          mirrorY: t > 0.5 ? endV.transform?.mirrorY : startV.transform?.mirrorY,
        },
        tintColor: t > 0.5 ? endV.tintColor : startV.tintColor,
        tintOpacity: startV.tintOpacity !== undefined && endV.tintOpacity !== undefined
          ? Number((startV.tintOpacity + t * (endV.tintOpacity - startV.tintOpacity)).toFixed(2))
          : startV.tintOpacity
      };
    } else if (startV?.active) {
      vstState = startV;
    } else if (endV?.active) {
      vstState = endV;
    }
  ...
  return {
    ...
    pins,
    vstState,
    smartCorrectState: startObj.smartCorrectState || endObj.smartCorrectState,
    smartWarp,
    cageState,
    meshState,
    transform3D,
