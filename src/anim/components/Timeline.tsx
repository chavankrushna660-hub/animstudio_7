  ...
  return (
    <div id="anim-timeline-container" style={style} className="bg-white border-0 p-3 shrink-0 flex flex-col gap-3 font-bold select-none overflow-y-auto text-black">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 bg-white border-0 p-1.5 rounded-2xl shadow-sm">
          <button
            id="timeline-play-btn"
            onClick={handlePlayPause}
            className={`p-2.5 rounded-xl transition-all cursor-pointer border-0 ${
              isPlaying 
          ...
          <button
            onClick={handleStop}
            className="p-2.5 rounded-xl bg-white hover:bg-neutral-100 text-black transition-colors cursor-pointer border-0"
            title="Stop & Reset to First Frame"
          >
            <Square className="w-5 h-5 fill-current stroke-[2.5]" />
          </button>
          <div className="w-[2px] h-7 bg-neutral-200 mx-1"></div>
          <button
            onClick={() => setLoopEnabled(!loopEnabled)}
            className={`p-2.5 rounded-xl transition-colors cursor-pointer border-0 ${
              loopEnabled 
        ...
        <div className="flex items-center gap-2 bg-white border-0 p-1.5 rounded-2xl shadow-sm">
          <span className="text-xs text-black font-black tracking-wider uppercase px-2 font-mono">{currentFrameIndex + 1}</span>
          <div className="w-[2px] h-7 bg-neutral-200 mx-1"></div>
          <button
            type="button"
            onClick={() => handleCopy(currentFrameIndex)}
            className="px-3 py-2 rounded-xl bg-white hover:bg-neutral-100 text-black transition-all text-xs flex items-center gap-2 cursor-pointer font-black border-0"
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-white hover:bg-neutral-100 text-black transition-all text-xs flex items-center gap-2 cursor-pointer font-black border-0 shadow-sm"
          ...
          {frames.length > 1 && (
            <button
              ...
              className="px-3 py-2 rounded-xl bg-white hover:bg-rose-50 text-rose-600 transition-all text-xs flex items-center gap-2 cursor-pointer font-black border-0"
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          <div className="flex items-center gap-1.5 bg-white border-0 p-1.5 rounded-2xl shadow-sm">
            <button
              onClick={() => setOnionSkinEnabled(!onionSkinEnabled)}
              className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-black transition-colors cursor-pointer border-0 ${
                onionSkinEnabled 
            ...
            >
              {onionSkinEnabled ? <Eye className="w-4.5 h-4.5 shrink-0 stroke-[2.8] !text-black" /> : <EyeOff className="w-4.5 h-4.5 shrink-0 stroke-[2.4]" />}
              <span className="text-xs font-black">ONION SKIN</span>
            </button>
            <button
              onClick={() => setOnionConfigOpen(!onionConfigOpen)}
              className={`p-2 rounded-xl bg-white hover:bg-neutral-100 text-black transition-colors cursor-pointer border-0 ${
          ...
          <div className="flex items-center gap-1 bg-white border-0 p-1.5 rounded-2xl shadow-sm">
            <button
            ...
            >
              <SlidersHorizontal className={`w-4.5 h-4.5 shrink-0 stroke-[2.8] ${autoTween ? '!text-black' : ''}`} />
          ...
          <div className="flex items-center gap-1 bg-white border-0 p-1.5 rounded-2xl shadow-sm">
            ...
            >
              <GitPullRequest className={`w-4.5 h-4.5 shrink-0 stroke-[2.8] ${showBones ? '!text-black' : ''}`} />
              <span className="text-xs font-black">{showBones ? 'HIDE BONES' : 'SHOW BONES'}</span>
            </button>
          </div>

          {/* AUTO FRAMES BUTTON */}
          <div className="flex items-center bg-white border-0 p-1.5 rounded-2xl shadow-sm">
            <button
              onClick={() => onToggleAutoFrames?.()}
              className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-black transition-all cursor-pointer border-0 ${
            ...
            >
              <Clock className={`w-5 h-5 shrink-0 stroke-[2.8] ${autoFramesActive || autoFramesStatus !== 'idle' ? '!text-black' : 'text-amber-500'}`} />
          ...
          <div className="flex items-center gap-1 bg-white border-0 p-1.5 rounded-2xl shadow-sm">
            ...
            >
              <Maximize className={`w-4.5 h-4.5 shrink-0 stroke-[2.8] ${showCanvasSizePanel ? '!text-black' : 'text-amber-500'}`} />
        ...
        <div className="flex items-center gap-2 sm:gap-3 bg-white border-0 px-3.5 py-1.5 rounded-2xl text-xs text-black shadow-sm">
          <span className="text-neutral-500 font-black tracking-wider uppercase hidden md:inline text-xs">SPEED</span>
          <div className="flex items-center gap-1.5">
            <button
              onClick={() => setFps(12)}
              className={`px-2.5 py-1 rounded-xl font-black text-xs cursor-pointer transition-colors border-0 ${
          ...
          />
          <span className="font-black text-black w-14 text-right text-xs font-mono">{fps} FPS</span>
        </div>
      </div>
      ...
      {autoFramesActive && (
        <div 
          ...
          className="w-full bg-white border-0 p-3.5 rounded-2xl shadow-lg flex flex-wrap items-center justify-between gap-3.5 animate-fade-in text-black shrink-0 z-30"
        >
          {/* Label and Badge */}
          <div className="flex items-center gap-2.5">
            <div className="flex items-center gap-2 bg-[#facc15] text-black px-3 py-1.5 rounded-xl font-black text-xs uppercase tracking-wider shadow-none">
              <Clock className="w-4.5 h-4.5 stroke-[2.6]" />
              <span>AutoFrames Seconds</span>
            </div>
            <span className="font-mono text-sm font-black text-black bg-neutral-100 px-3 py-1.5 rounded-xl shadow-none">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-xs text-neutral-500 font-black uppercase tracking-wider mr-1 hidden sm:inline">Presets:</span>
            {[2, 3, 5, 10, 15].map((sec) => (
              <button
                key={sec}
                type="button"
                onClick={() => setAutoFramesDelay?.(sec)}
                className={`h-9 px-3.5 rounded-xl text-xs font-black transition-all cursor-pointer border-0 ${
                  autoFramesDelay === sec
          ...
          <div className="flex items-center gap-2.5 min-w-[180px] sm:min-w-[220px] flex-1 max-w-xs">
            <span className="text-xs text-neutral-500 font-black">2s</span>
            <input
              type="range"
              min="2"
              max="15"
              ...
              className="w-full accent-amber-500 cursor-pointer h-2.5 rounded-lg bg-neutral-200"
            />
            <span className="text-xs text-neutral-500 font-black">15s</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            {autoFramesStatus === 'idle' ? (
              <button
                ...
                className="h-10 px-4 bg-white hover:bg-neutral-100 text-black font-black text-xs uppercase rounded-xl tracking-wider transition-all flex items-center justify-center gap-2 cursor-pointer shadow-sm border-0 active:scale-95"
              >
                <Play className="w-4 h-4 fill-current stroke-[2.4]" />
                Start Auto Frames ({autoFramesDelay}s)
              </button>
            ) : autoFramesStatus === 'countdown' ? (
              <div className="h-10 px-4 flex items-center justify-center font-black text-black text-xs bg-neutral-100 rounded-xl border-0 animate-pulse">
              ...
              <div className="flex items-center gap-2">
                {autoFramesStatus === 'recording' ? (
                  <button
                    ...
                    className="h-10 px-3.5 bg-white hover:bg-neutral-100 text-black font-black text-xs rounded-xl flex items-center justify-center gap-1.5 border-0 cursor-pointer shadow-sm"
                  >
                ...
                <button
                  ...
                  className="h-10 px-3.5 bg-rose-600 hover:bg-rose-500 text-white font-black text-xs rounded-xl flex items-center justify-center gap-1.5 border-0 cursor-pointer shadow-sm"
                >
      ...
      <div className="flex items-center gap-2.5 overflow-x-auto py-1.5 pr-12 scrollbar-thin select-none">
        {frames.map((frame, index) => {
          ...
          return (
            <div
              ...
              className={`group min-w-[76px] h-20 rounded-2xl border-0 flex flex-col justify-between p-2.5 cursor-pointer transition-all relative shrink-0 shadow-sm ${
                isActive
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <span className={`text-xs sm:text-sm font-mono ${isActive ? 'text-black font-black' : 'text-neutral-700 font-bold'}`}>
                  {index + 1}
                </span>
                {frame.objects && Object.keys(frame.objects).length > 0 && (
                  <span className="w-2.5 h-2.5 rounded-full bg-amber-500 shadow-sm" title="Has keyframe transforms"></span>
                )}
              </div>

              {/* Indicator info */}
              <span className="text-[10px] text-neutral-500 block text-right select-none font-black font-mono">
        ...
        <button
          ...
          onClick={addFrame}
          className="min-w-[76px] h-20 rounded-2xl border-0 bg-yellow-300 hover:bg-yellow-200 flex items-center justify-center text-black transition-all cursor-pointer shrink-0 shadow-sm active:scale-95"
          title="Add New Frame"
        >
        ...
        <div className="flex items-center gap-2 bg-white border-0 p-2.5 rounded-2xl shrink-0 h-20 shadow-sm">
          <div className="flex flex-col justify-center min-w-32">
            <span className="text-[10px] text-black font-black uppercase tracking-wider mb-1">Batch Add</span>
            <CustomSelect
          ...
          <button
            ...
            className="h-9 px-4 rounded-xl bg-white hover:bg-neutral-100 text-black font-black text-xs transition-all flex items-center justify-center self-end cursor-pointer shadow-sm border-0 active:scale-95"
          >
            ADD
          </button>
        </div>
